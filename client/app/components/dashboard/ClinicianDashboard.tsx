...
const ClinicianDashboard = () => {
  ...
  useEffect(() => {
    if (isConnected && address) {
      // TODO: Replace this with actual smart contract call to check NFT
      setTimeout(() => {
        setIsVerified(true); // Simulate verified status
        setIsActive(true); // Simulate active status
        setHasPendingRequests(Math.random() > 0.5); // Random demo data
        setLoading(false);
      }, 1500);
    }
  }, [isConnected, address]);
      ...
      <AnimatePresence>
        ...
        >
          ...
          <p className="text-gray-500 max-w-md">
            Please connect your wallet to access the clinician dashboard
          </p>
        </motion.div>
      </AnimatePresence>
    );
      ...
      <AnimatePresence>
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <p className="text-gray-500 max-w-md">
              You need to be a verified clinician to access this dashboard
            </p>
          </div>
  ...
  return (
    <div className="min-h-screen w-[90%] mx-auto bg-gray-50 dark:bg-gray-900 p-6">
      ...
      >
        ...
        <div className="flex items-center space-x-3">
          <div className={`px-3 py-1 rounded-full text-sm font-medium flex items-center ${isActive ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-300'}`}>
            <div className={`h-2 w-2 rounded-full mr-2 ${isActive ? 'bg-green-500' : 'bg-gray-500'}`} />
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold mt-1">{hasPendingRequests ? "2" : "0"}</h3>
            </div>
          ...
          <div className="mt-4 flex items-center text-sm text-green-500">
            <ArrowPathIcon className="h-4 w-4 mr-1" />
            <span>{hasPendingRequests ? "New requests available" : "No pending requests"}</span>
          </div>
        </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        >
          <div className="flex items-center space-x-4 mb-4">
            <div className={`p-3 rounded-lg ${isActive ? 'bg-green-100 dark:bg-green-900/20' : 'bg-gray-100 dark:bg-gray-700'}`}>
              <UserIcon className={`h-6 w-6 ${isActive ? 'text-green-600 dark:text-green-400' : 'text-gray-500'}`} />
            </div>
          ...
          <motion.button 
            whileHover={{ backgroundColor: isActive ? "#ef4444" : "#10b981" }}
            onClick={() => setIsActive(!isActive)}
            className={`w-full px-4 py-3 rounded-lg text-sm font-medium text-white ${isActive ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600'}`}
          >
